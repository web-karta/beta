...
o._allowedAtStart = tripAllowedNow(o, o._t0);
  ...
  function pickRouteKeyForTrip(trip) {
    const line = trip.linija;
    const term = trip.okretište;
    const code = TERM_CODE[term];
    if (!code) return null;
    ...
    if (trip.smjer.includes('prema')) {
      const exact = candidates.filter(k => k.endsWith('-' + code));
      return exact[0] || null;
    }
    if (trip.smjer.includes('od')) {
...
function popupHtml(tr, state) {
  const dest =
    (state.mode === 'waiting' && !state.inDepot)
      ? oppositeDestFromRouteKey(state.routeKey || '')
      : destFromRouteKey(state.routeKey || '');
...
if (!arrAllowed.length && !activeAny) {
  ...
  // ✔️ inače (još unutar 5 min) → pusti render dalje
      ...
      const { prev, next } = findPrevNext(arrAllowed, t);
...
if (active) {
  ...
  // 🔒 AKO VEĆ IMAMO RUTU ZA OVO VOZILO – NE PREPISUJ
  rk = lastRouteKeyByVehicle.get(vozilo);

  // samo na SAMOM POČETKU vožnje je postavi
  if (!rk) {
    rk = pickRouteKeyForTrip(active);
...
const prevKey =
  ...
  const prevEndsDepot = isDepotEnd(prevKey);
...
// 🚋 OKRETIŠTE → STOJI (I AKO NEMA VIŠE VOŽNJI)
if (!prevEndsDepot) {
  pos = endPos;

  const textKey = prevKey;                       // tekst: "PEŠIJA"
  const nextKey = next ? pickRouteKeyForTrip(next) : null; // ruta ako postoji
  ...
  popupState = {
    mode: 'waiting',
    secondsLeft: next ? (next._t0 - t) : null,   // ⬅️ KLJUČNO
    routeKey: textKey,
    networkRouteKey: nextKey || textKey,         // ⬅️ fallback
    nextStopName: null
  };
}

  // 🏠 SPREMIŠTE → STOJI SAMO DEPOT_POST
...
// ❌ NIKAD NE BRIŠI AKO POSTOJI AKTIVNA VOŽNJA
if (!pos || !trForLabel || !popupState || !popupState.routeKey) {

  // ako je vozilo u aktivnoj vožnji → zadrži marker
  if (activeAny) {
    // fallback pozicija
    const fallbackPos =
      lastPosByVehicle.get(vozilo) ||
      (lastRouteKeyByVehicle.get(vozilo)
        ? buildRoute(lastRouteKeyByVehicle.get(vozilo))?.poly?.[0]
        : null);
    ...
    if (fallbackPos) {
      ...
      popupState = popupState || {
        mode: 'moving',
        secondsLeft: activeAny._t1 - t,
        routeKey: lastRouteKeyByVehicle.get(vozilo),
        networkRouteKey: lastRouteKeyByVehicle.get(vozilo),
        nextStopName: null
      };
    } else {
      // nema još ničega → NE DIRAJ marker ovaj tick
      continue;
    }
  } else {
    // nema aktivne vožnje → stvarno briši
    if (existing) {
      layer.removeLayer(existing);
      markers.delete(vozilo);
    }
